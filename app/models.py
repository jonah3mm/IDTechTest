...
class Asset(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    audit_logs = db.relationship('AssetStatusChange', backref='asset', lazy=True, cascade='all, delete-orphan')
...
class AssetStatusChange(db.Model):
    __tablename__ = 'asset_status_changes'

    id = db.Column(db.Integer, primary_key=True)
    asset_id = db.Column(db.Integer, db.ForeignKey('assets.id'), nullable=False)
    previous_status = db.Column(db.String(50), nullable=False)
    new_status = db.Column(db.String(50), nullable=False)
    requester_ip = db.Column(db.String(100), nullable=False)
